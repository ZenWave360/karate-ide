    ...
    public provideHover(
        ...
        if (textLine.trim().startsWith('|')) {
            ...
            if (previousTextLine.trim().startsWith('|')) {
                ...
                const debugArgs = encodeURIComponent(JSON.stringify([document.uri.fsPath, position.line + 1]));
                contents.appendMarkdown(`[Karate: Run](command:karateIDE.tests.run?${debugArgs} "Karate: Run")`);
                contents.appendMarkdown(' | ');
                contents.appendMarkdown(`[Karate: Debug](command:karateIDE.tests.debug?${debugArgs} "Karate: Debug")`);
