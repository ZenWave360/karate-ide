...
export async function configureClasspath() {
    ...
    const items = [
        ...
        },
        {
            ...
            detail: 'Requires karate dependencies to be present in your local maven repository.',
            value:
    ...
    if (answer) {
        ...
        if (answer.label === 'KarateIDE Classpath Jar') {
            const karateJar = vscode.Uri.joinPath(vscode.Uri.file(classpathJarExtension.extensionPath), 'karate.jar').fsPath.replace(/\\/g, '/');
