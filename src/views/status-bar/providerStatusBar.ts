...
class ProviderStatusBar {
    ...
    private static statusBarCommandId: string = 'karateIDE.tests.showExecutionHistory';

    constructor(context) {
        ProviderStatusBar.statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 50);
        ProviderStatusBar.statusBarCommand = vscode.commands.registerCommand(
            ProviderStatusBar.statusBarCommandId,
            ProviderExecutions.showExecutionHistory
        );
        ...
        ProviderResults.onResultsJson(json => {
            ProviderStatusBar.setFromResults(json);
        });
    }
    ...
    private static setFromResults(json) {
        ...
        let resultsStats: string = `Features: ${json.features} | Scenarios: ${json.scenarios} | Passed: ${json.passed} | Failed: ${
            json.failed
        } | Elapsed: ${(json.elapsedTime / 1000).toFixed(2)}`;
