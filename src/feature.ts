...
class Feature {
    ...
    public getPositionalTokens(tokens: ILineToken[], position: vscode.Position): ILineToken[] {
        ...
        for (let ndx = 0; ndx < tokens.length; ndx++) {
            if (tokens[ndx].startIndex <= charPosition && tokens[ndx].endIndex >= charPosition && !this.isKeyword(tokens[ndx].text.toLowerCase())) {
    ...
    public getSectionType(line: string): string {
        if (this.isBackgroundSection(line)) {
            return SECTION_TYPE.BACKGROUND;
        }
        if (this.isFeatureSection(line)) {
            return SECTION_TYPE.FEATURE;
        }
        if (this.isScenarioSection(line)) {
            return SECTION_TYPE.SCENARIO;
        }
        if (this.isScenarioOutlineSection(line)) {
            return SECTION_TYPE.SCENARIO_OUTLINE;
        }
        if (this.isExamplesSection(line)) {
            return SECTION_TYPE.EXAMPLES;
        }
        if (this.isTagSection(line)) {
            return SECTION_TYPE.TAG;
        }
        if (this.isCommentSection(line)) {
            return SECTION_TYPE.COMMENT;
        }
    ...
    public isKeyword(word: string): boolean {
        ...
        if (steps.includes(word.toLowerCase())) {
            return true;
        }
        ...
        if (asserts.includes(word.toLowerCase())) {
            return true;
        }
