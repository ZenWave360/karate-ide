...
export class ProviderBuildReports implements vscode.TreeDataProvider<IEntry>, vscode.FileSystemProvider {
    ...
    async getChildren(element?: IEntry): Promise<IEntry[]> {
        ...
        let buildReportFiles = await vscode.workspace.findFiles(glob).then(value => {
            return value;
        });
        ...
        if (element) {
            ...
            if (displayType === 'Shallow') {
                ...
                return buildReportFilesFiltered.map(reportFile => ({ uri: reportFile, type: vscode.FileType.File }));
            } else {
